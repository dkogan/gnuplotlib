...
draw a colormapped grid of, we can do this as a 2D plot. Let's do that, and also
...
This is very useful for annotating images.
...
and have it bin it for us, or we can do it with =numpy=. Let's sample a normal
distribution, and plot the histogram. With =gnuplotlib=:
