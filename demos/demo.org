...
- with points sampled around the edge of the ellips (possible in 2D and 3D)
...
can work in 3D too (we can remap an ellipse). I'm using the same data here, so
the points should trace the same shape as the ellipses I just computed
