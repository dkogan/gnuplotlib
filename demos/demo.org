...
how much data is received. Example. Let's color-code the sinusoid.
...
** Specifying multiple datasets together
Usually we want to plot multiple datasets in the same plot. Note that the code
and documentation uses the terms "dataset" and "curve" interchangeably.
...
Let's plot a sine and a cosine together, using the default styling for one
(including no legend), and a specific styling for another. And let's set some
common options.
...
those specific options. For instance, if I want to plot logs of things with
...
=plot()= call. It's supported across the data arguments /and/ the curve options.
Often this lets you simplify the =plot()= command significantly. Previously we
plotted two sinusoids together using a tuple for each dataset. With
broadcasting, we can avoid that:
...
=(2,100,)= for the y. The broadcasting logic kicks in, and we plot two separate
datasets, one for each row of y. The curve options broadcast as well. The
=legend= is expecting a scalar, but I gave it an array of shape =(2,)=, so it
uses a different legend for each of the two plotted datasets.
